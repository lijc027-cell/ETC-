...
> 状态：草案
> 目标：把 ETF 查询收束为统一的 Text-to-Query AST 编译链，在保持 v1 回归稳定的基础上，分阶段扩展搜索、筛选、对比、base 扩展字段和报告展开能力。
...
PM 文档里的业务桶是覆盖分组，不是运行时函数分发入口。v3 仍然是统一 text-to-AST 系统：`query_mode` 负责外层路由，`intent` 只作为受限语义标签，`field_profile` / `answer_fields` 负责字段展示约束，最终都必须落到同一条 AST -> Mongo 编译链路。能力注册表只提供约束、白名单和 gate，不承担业务函数分发。
...
- 最小 Executable Capability Registry：替代散落常量，作为字段白名单、格式、gate 和安全约束的唯一来源，不作为业务语义分发中心
...
v3.2 的目标是把 v3.0 / v3.1 已覆盖并验收通过的 executable questions 全部迁移到统一的 LLM full AST Draft 路径，同时新增 v3.2 base 扩展能力。
...
### v3.3 定位：v3.2 Base AST 的结构化扩展层

v3.3 不是 report 专用协议、derived performance 专用协议、profile dispatch 层或业务 handler 分发层。v3.3 是 `v3.2 base AST` 的向后兼容结构化扩展：v3.2 标量 AST 在 v3.3 中仍是合法子集，且继续走同一条执行链：

```text
Draft AST -> validator -> compiler -> executor -> formatter
```

v3.3 顶层 schema 可命名为 `v3.3-structured-query`，但它的含义只能是“v3.2 AST extension”。`v3.3-derived-performance`、report expansion、array timeline、timeseries latest/latest_two/specified、multi-child composite 都只能作为 v3.3 统一 AST grammar 下的子 schema/profile 或 grammar fragment，不能成为 v3.3 顶层协议。

兼容规则：

- v3.2 base AST 可保留 `report_period=null`、`expand=null`、`timeseries_semantics=null` 等兼容字段。
- v3.2 base AST 不需要携带 v3.3 子能力的非空结构化语义，例如非空 report expansion、非空 `timeseries_semantics` 或 derived return declaration。
- 非当前 grammar 使用的字段必须允许缺省或为 `null`；但任何非空结构都必须由 LLM Draft AST 显式表达，并通过 schema/profile/Registry 约束校验。
- 任一子 grammar 的 required fields 不得被提升为 v3.3 顶层 required fields。
- v3.3 validator 必须按 `schema_version + AST shape + profile constraints` 校验，不得要求所有 v3.3 AST 都符合某个子能力 schema。

职责边界：

- LLM Draft AST 表达用户语义。
- Registry 是可执行约束真源：phase、gate、field、collection、operator、format、schema/profile 和安全边界。
- parser/evidence 只生成 generation context，不能直接生成 strict-pass semantic AST。
- validator 只做结构校验、非语义归一、identity/value normalization、安全 cap 和 provenance 记录；不得补业务语义。
- compiler 把 Validated AST 编译为 Mongo 查询或受控 execution plan。
- executor 只执行 compiled plan 指定的只读查询和受控计算。
- formatter 只展示 executor result 与 compiled metadata，不派生查询语义。

v3.3 grammar 范围包括：report `report_period + expand`、array timeline、timeseries `latest/latest_two/specified`、multi-child composite、derived performance。任一 grammar 能力进入 release gate 前，必须同时具备 AST shape、Registry gate、coverage、strict-pass 和 provenance 标准；口径未验证能力只能写成 `planned blocked` 或 future grammar application。

#### v3.3 Release Boundary [Hard]

v3.3 是 unified AST grammar extension release，不是“所有 ETF 问答能力完成”的全能力 release。release pass 只能针对 verification 前已经声明的 release scope 判断，不得用单个子 grammar、单个 profile 或单次 smoke 结果冒充 v3.3 release。

规则：

- v3.3 release scope 必须在 verification 前由 coverage matrix、release checklist 或 gate manifest 声明。
- 任一单独 grammar fragment pass 只能声明 fragment pass，不能声明 v3.3 release pass。
- `v3_2_compatibility_baseline` 必须从 v3.2 declared executable capabilities 派生，并按原 grammar/query mode 分组；不得手写固定能力列表。
- `v3_3_new_fragments` 至少按 declared grammar fragments 分桶，例如 `report`、`timeseries`、`derived_performance`、`composite`。
- planned blocked / future grammar application 不进入 release denominator，也不得标成 executable success。
- v3.2 base AST 是 v3.3 合法子集；向后兼容失败属于 architecture nonconformance。

#### v3 Architecture Invariants [Hard]
...
非 AST 路径只能产生 deny / clarification / blocked / unsupported，不得产生业务答案。

真源边界：

- semantic truth source 是 Draft AST。
- constraint truth source 是 Registry。
- execution truth source 是 compiled plan。
- display truth source 是 executor result + compiled metadata。

compiler rule selection 只能检查 Validated AST shape 和 Registry constraints。`intent`、`profile`、`schema_version`、`output_style` 可用于约束校验和消歧，但不得作为唯一 compiler selector。即使函数参数是 Validated AST，`if intent/profile/schema_version then business compiler` 仍不符合本规格。

#### Versioning and Compatibility Policy [Hard]

Runtime `ast_schema_version` 只允许协议层级值，例如 `v3_2_base_ast`、`v3_3_structured_query`。

禁止 capability-level top schema_version，包括但不限于：

- `v3.3-derived-performance`
- `v3_3_report_ast`
- `v3.3-report-array`

规则：

- validator 不得根据 capability-level schema_version 加载顶层 required schema。
- `grammar_fragment_id`、`compiler_rule_id`、`profile` 可作为 metadata / audit / constraints / disambiguation。
- 这些 metadata 不得成为 top-level schema_version，也不得作为唯一 execution selector。
- v3.3 新 grammar required fields 只在对应 AST shape 激活后 required；任一子 grammar 的 required fields 不得提升为所有 v3.3 AST 的 required fields。
- `v3_2_base_ast` 必须在 v3.3 runtime 中保持合法；无法校验或执行 v3.2 base AST 是 architecture nonconformance。

#### Sub-Grammar Composition Model [Hard]

grammar fragment 是 AST schema fragment + compiler rule set，不是 handler、service entry、processor、adapter 或业务入口。

规则：

- 单 AST 只能有一个 `from`；跨 collection 必须拆成 multi-child composite。
- `from=tb_ths_etf_base` 同时出现非空 `report_period` 或 `expand` 必须 validation failure。
- report `expand` 与 base collection `timeseries_semantics` 出现在同一 AST 必须 validation failure。
- 单 child AST 同时要求跨 collection 字段必须 validation failure。
- report array = report collection + 非空 `report_period` + 非空 `expand`。
- report scalar = report collection + 非空 `report_period` + `expand=null`。
- base timeseries strict pass 必须有 Draft AST 显式 `timeseries_semantics`。
- derived performance = base collection + explicit derived return declaration + period。
- composite child 可是普通 scalar、report、timeseries、derived performance，但每个 child 必须独立 Draft AST / validation / compiler / provenance。
...
- 用户未显式说时间且 LLM 完全未输出 `timeseries_semantics` 时，validator 可对所有被选中的时间序列字段补默认 `latest`，并记录 `validator_applied_default`；这只允许用于本规格明确列出的 legacy compatibility display behavior，不计入 v3.3 `base_timeseries` grammar strict pass。
...
- v3.3 timeseries grammar strict pass 中，用户请求的 `latest` / `latest_two` / `specified` 语义必须来自 Draft AST。
- validator default 的 timeseries semantics 不得计为 v3.3 `base_timeseries` grammar strict pass。
...
### v3.3 Derived Performance Explicit Semantics [Hard]

`grammar_fragment` 是独立于 `query_mode` 的可复用查询协议片段：它定义 AST shape、Registry 约束、compiler 行为、executor/derived phase 契约、formatter 输入输出和 provenance/audit 要求。`derived_performance` 是 v3.3 的 grammar fragment，不是顶层 schema、业务 handler、profile dispatch 或 formatter 模板。

v3.3 strict performance 的收益率语义统一通过 `derived_performance` fragment 执行。收益率值只允许从 `ths_unit_nv_fund` 的完整交易日序列派生；`ths_yeild_*` 收益率标量字段不得作为 v3.3 strict performance 的 select、where、order_by、compare baseline 或 formatter 输入来源。`ths_yeild_rank_*` 排名字段仍是数据库物理字段。

derived performance 不能从 `intent=performance`、`profile=derived_performance_table`、`output_style=performance_table` 推导 derived metric。Draft AST 必须显式声明 derived return alias、`type=derived_return`、period 和对应 `select` / derived declaration。

规则：

- profile 注册：
  - `derived_performance_table`：只用于单实体 performance table，默认 `collection=tb_ths_etf_base`。
  - `derived_return_list`：用于收益率 filter/sort/topN，默认 `collection=tb_ths_etf_base`。
  - `rank_list`：只用于明确排名/名次语义，默认 `collection=tb_ths_etf_base`。
  - compare/composite 不使用 `derived_performance_table` table profile；它们保留自身 profile/output style，并激活 `grammar_fragment=derived_performance` 作为单 period metric block。
- output style 注册：
  - `derived_performance_table` 使用 `performance_table`。
  - `derived_return_list` 使用 `list`。
  - `rank_list` 在 filter/list 场景使用 `list`，single 场景使用 `summary`。
  - derived compare 沿用现有 `compare` output style。
- 非 composite 的 derived performance AST 中，`sub_intents=null`、`report_period=null`、`timeseries_semantics=null`、`expand=null`。`timeseries_semantics=null` 表示 LLM 不得用 timeseries grammar 表达收益率计算。
- `select` 中的 derived alias 必须写成 object：`{"alias": "return_1y", "type": "derived_return", "period": "1y"}`。`return_*` 命名前缀和 `type=derived_return` 必须一致，否则 validator 拒绝。
- `select.type=derived_return` 是 semantic derived alias，compiler 不加入 Mongo projection。compiler 只把它编译为 derived phase metric。
- `answer_fields.source` 默认为 `mongo`，普通物理字段可省略；`return_*` 必须显式 `source=derived`。validator 必须拒绝 source/field 类型不匹配。
- `ths_unit_nv_fund` 不允许由 LLM 在 `select` 或 `answer_fields` 中请求；它只能通过 Registry 的 `required_physical_inputs` 由 compiler 追加。
- v3.3 performance profiles 必须用 `required_physical_inputs=["ths_unit_nv_fund"]` 声明 NAV 依赖。compiler 必须追加完整 `ths_unit_nv_fund` 数组 projection，禁止 `$slice:-1`。
- `required_physical_inputs` 不是 LLM semantic select，也不是 `timeseries_semantics`。同一 execution plan 中，同一字段不能同时作为 `required_physical_inputs` 和 `display_context_fields`。
- `display_context_for_period_range` 只属于 v3.0-v3.2；v3.3 derived performance 的时间范围由 derived phase 输出。
- `performance_rows` 是 display contract field，不是 query semantics source。
- validator、compiler、formatter 不得用 `performance_rows` 反推 `select` 或 derived declaration。
- `select` / derived declaration 与 `performance_rows` 必须一致；缺失或冲突即 validation failure。
- Validated AST 输入的 `performance_rows` 形态为 `[{alias, period, label}]`。derived phase 输出的 metadata 形态为 `[{alias, period, label, null_reason, start_btime, end_btime, display_range}]`。收益率值只存在于 result row 的 alias 字段；`performance_rows` 只提供 metadata。
- `performance_rows[].label` 由 LLM Draft 生成。validator 可按 Registry canonical label 归一并记录 normalization；这不算 semantic repair。formatter 直接使用 `performance_rows[].label`，不得自行生成 label。
- 每个 `performance_rows[].alias` 必须对应 `select` 中一个 `type=derived_return` alias；每个 selected derived return 必须有且仅有一个 performance row metadata；两边 period 必须一致。
- compiler 可追加 NAV/rank physical dependencies，但必须记录 provenance，不能补 Draft AST 缺失语义。
- derived performance strict pass 的 semantic provenance 必须能从 Draft AST 追踪到 compiled `derived_phase.metrics`。
- single table 至少包含 `fundcode`，可含基金简称；list 至少包含 `fundcode`、基金简称、selected return/rank fields；compare 必须有 `fundcode` 作为行或列标识，`return_*` aliases 作为 metric 字段。
- `return_*` derived alias 支持 `gt,gte,lt,lte`。`eq` 仅当用户明确输入精确数值等值（如“等于 20.00%”）时允许；“约/左右”必须 clarification 或由未来范围规则处理。
- `period=all` 只允许单只基金 performance table：由 LLM Draft 的 all/全部周期语义归一展开 10 个 `return_*` aliases，并在 provenance 中标记 normalization from `period=all`。compare/list/filter/topN 遇到 `period=all` 必须返回 `ClarificationRequired(reason=multi_period_compare_unsupported)`，要求用户选择单一周期或改为单只基金各周期表。
- compare 中的 derived performance 只允许单 period `return_*` metric block，不允许 `performance_table` 多周期行结构。显式 compare 中若包含收益率维度，compiler 使用 `fundcode in [...]` 拉取每个 fundcode 的 Mongo 直通字段、排名字段和完整 `ths_unit_nv_fund` 数组；derived phase 以 `fundcode` 为 key 独立计算 `return_*`，合并到每个基金行；formatter 按现有 compare 表格渲染。
- composite_single 的 performance child 必须独立经过 compiler、Mongo、derived performance phase。filter -> compare 只传 fundcode 集合；显式 compare 收益率维度也必须走 derived performance，不能用旧 baseline。
- gate blocked 返回 `UnsupportedQuery(blocked_by_verification)`，不进 prompt/Mongo。gate passed 但 NAV 缺失或非法时，进入 Mongo 后按 null / `data_not_available` 策略处理。
...
上例是 v3.1/v3.2 legacy performance 字段视图。v3.3 strict performance 的 Registry 行必须使用 `derived_performance` fragment、`return_*` derived alias catalog 和 `required_physical_inputs`，不得把 `ths_yeild_*` 收益率标量作为收益率语义字段。
...
- Registry 是约束真源，不是用户语义真源；用户最终查询语义只能来自 Draft AST。
- `intent` 是 AST/profile/output 约束标签，不是执行指令、handler key 或字段模板入口。
- `intent_candidates` 是 pre-AST routing 的候选集合，只能用于 prompt 裁剪和 Registry gate 过滤；最终查询语义以 Draft AST 为准。
- period/filter/TopN/report_period/timeseries evidence 只能帮助 LLM 生成 AST，不能直接生成 strict-pass semantic AST。
...
- Intent Recognition 是执行路由的唯一权威来源；LLM AST 只能补全查询结构，不能改变 `recognized_query_mode`、扩大 intent/from 候选范围，不能绕过 deny/clarify。它不是业务函数分发器，`intent` 也不是命令式执行开关。
...
```json
{
  ...
  "registry_hash": null,
  "ast_schema_version": "v3_2_base_ast | v3_3_structured_query | null",
  "model_id": null,
  "retry_count": 0,
...
- `ast_schema_version` 只允许取 `v3_2_base_ast` 或 `v3_3_structured_query` 两层；具体子能力由 AST shape 表达，不得把 `v3_3_report_ast`、`v3.3-derived-performance` 等子 grammar 名称作为 v3.3 顶层 schema。
...
上例展示 v3.2 legacy scalar field 的 selection_context 形态。v3.3 strict performance 中，`selectable_fields` 的收益率语义应为 `return_*` derived alias catalog；`ths_yeild_*` 收益率标量只保留为 v3.0-v3.2 legacy field catalog。
...
| 收益率超过20% | v3.2: `ths_yeild_1y_fund`; v3.3: `return_1y` derived alias | gt | 20 |
| 收益超过20% | v3.2: `ths_yeild_1y_fund`; v3.3: `return_1y` derived alias | gt | 20 |
...
`llm_ast_draft` 路径中，LLM 必须输出完整 AST Draft。v3.2 base AST 的兼容顶层字段为：
...
v3.2 base AST 中缺少任一兼容顶层字段均为 `schema_validation` failure。

v3.3 继承 v3.2 base AST，并只按 AST shape 引入子 grammar 结构。`schema_version=v3.3-structured-query` 不表示所有 AST 都必须携带 report、timeseries、composite 或 derived performance 的非空字段；v3.2 标量 AST 仍是合法子集。子 grammar 的 required fields 只在对应 AST shape 出现时生效：

- report array shape：非空 `report_period` + 非空 `expand`。
- report scalar shape：非空 `report_period` + `expand=null`。
- timeseries shape：被 `select`/`where`/`order_by` 使用的字段为 `[{value,btime}]`，并存在非空 `timeseries_semantics.by_field`。
- multi-child composite shape：父级 composite bundle + 每个 child 独立 AST。
- derived performance shape：Draft AST 显式声明 derived return metric + period，并由 derived performance profile 约束。

未命中某个子 shape 时，该子 grammar 的 required fields 不得被提升为顶层 required fields。任何非空子结构都必须来自 Draft AST，并通过 Registry/profile/schema 校验；validator 不得因为 `schema_version` 或 `profile` 自动补出非空子结构后计为 strict pass。
...
上例是 v3.2 base AST legacy 示例。v3.3 strict performance 必须把收益率 select 改为 `{"alias":"return_1y","type":"derived_return","period":"1y"}`，并在 `answer_fields` 中显式 `source=derived`。
...
- `schema_version` 和 `profile` 只用于约束校验与消歧，不能覆盖 AST shape 或单独决定执行路径。
- profile 不得作为唯一 execution-path selector；编译规则选择必须以 Validated AST shape 为主。
...
`performance_period_range` 是 v3.0-v3.2 的 display expansion 约束：compiler 可读取 `ths_unit_nv_fund` 的 `btime` 序列计算时间范围 metadata，但不得用净值序列重算收益率。**v3.3 起**，derived performance 在 post-processing 阶段从 `ths_unit_nv_fund` 交易日序列计算收益率和时间范围，不在此约束范围内；formatter 只能消费 compiler/derived phase 返回的结果，不自行计算。

### Provenance 与 Failure 规则 [Hard]

executable strict pass 必须保留 Draft AST、Validated AST、compiled query/plan provenance。strict pass 必须证明 Draft AST、Validated AST、Compiled Plan 三段语义一致，且没有 semantic repair。

规则：

- post-compiled 或 post-execution 的 `data_not_available` 仍保留已生成 provenance，包括 Draft AST、Validated AST 和 compiled plan。
- pre-AST deny、clarification、blocked_by_verification 不强制 compiled provenance；这些结果必须明确 `ast_required=false` 或未进入 compiler。
- partial composite data missing 必须保留 parent AST 与每个 child query 的执行状态；单个 child 的 `data_not_available` 不得抹掉其它 child 的 provenance。
- 删除 Draft AST 中用户请求的 semantic field、period、order_by、report_period、expand、timeseries_semantics 或 derived return declaration 后，validator/compiler/formatter 不得修复后计为 strict pass。
- 若修复发生在 validator、compiler、executor 或 formatter 任一阶段，必须记录为 semantic repair，且 `strict_pass=false`。
...
`performance` 在 v3.2 中必须走 `llm_ast_draft`。period 归一可以作为 deterministic evidence 提供，但不得直接生成 select 字段或完整 AST；LLM 必须仍显式选择 Registry 允许的 period-specific fields。v3.3 strict performance 不再选择 `ths_yeild_*` 收益率标量字段，而是选择 `return_*` derived alias object；本节中 `ths_yeild_*` candidate field 示例仅适用于 v3.2 legacy field catalog。
...
```json
{
  ...
  "candidate_fields": [
    {"field": "ths_yeild_ytd_fund", "evidence": "今年", "phase": "v3.2_legacy"},
    {"field": "return_ytd", "type": "derived_return", "period": "ytd", "evidence": "今年", "phase": "v3.3_strict"},
...
- LLM 选择的收益率 / 排名字段必须能追溯到 period evidence。v3.3 中收益率字段指 `return_*` derived alias，排名字段仍指 `ths_yeild_rank_*` 物理字段。
...
- search result fundcode set 只有在 `search_has_more=false` 时才可作为 v3.3 derived filter/sort/topN 的完整 bounded universe；否则后续 derived filter/sort/topN 不得 strict pass。runtime 必须记录 search query provenance、fundcode set、search cap、`search_has_more`。
...
```json
{
  "field_profile": "filter_list | derived_return_list | rank_list",
  "selectable_fields": [
    "fundcode",
    "ths_fund_extended_inner_short_name_fund",
    "ths_fund_scale_fund",
...
v3.3 strict derived performance 中，收益率 filter/sort/topN 使用 `derived_return_list`，`selectable_fields` 中的收益率语义字段应是 `return_*` alias catalog，而不是上述 v3.2 legacy `ths_yeild_*` 标量字段。排名/名次问法使用 `rank_list`，只选择 rank physical fields。
...
v3.3 `return_*` derived alias 的 `eq` 只允许用户明确输入精确数值等值（如“等于 20.00%”）。自然语言中的“约 / 左右 / 差不多”不得编译为 `eq`，必须返回 clarification 或等未来范围规则开放后转为范围查询。
...
```text
physical fields referenced by select
+ where.field
+ order_by.field
...
`display_context_fields` 只能来自 Registry 明确声明，例如 v3.0-v3.2 `performance` 的 `ths_unit_nv_fund` 用于 `performance_period_range`。formatter 或 compiler 不得临时推断新的 display context field。v3.3 derived performance 使用 `required_physical_inputs` 声明 `ths_unit_nv_fund`，不得同时把它列为 `display_context_fields`。
...
- `performance_period_range` 是 v3.0-v3.2 受控 display expansion：compiler 可读取 `ths_unit_nv_fund` 的 `btime` 序列计算时间范围 metadata，但不得用净值序列重算收益率、不得改变 semantic filter/order/limit。v3.3 derived performance 在 compiler 之后的 derived phase 执行收益率计算，compiler 仅负责按 `required_physical_inputs` 取回完整 `ths_unit_nv_fund` 原始数组和排名字段，禁止 `$slice:-1`。
- 当 `where` 或 `order_by` 使用 `return_*` 时，Mongo phase 不得先按用户 `limit` 预截断候选再执行 derived filter/sort。用户 `limit` 来自 Draft，表示返回条数，只能在完整候选集或已证明闭集的 bounded universe 上完成 derived filter/sort 后应用。
- `execution_cap` / `candidate_cap` 是安全上限，不得伪装成用户 limit。若候选集超出 cap 且不能证明闭集，运行时只能返回 non-strict partial result。

### AST Shape Compiler Registry [Hard]

如需 dispatcher，只能定义为 validation 后的 AST shape compiler registry。该 registry 不是业务 handler、服务入口或语义识别入口。

规则：

- AST shape 是编译规则选择主依据，由适用的顶层结构、非空结构字段、`where`、`order_by`、`expand`、`timeseries_semantics`、child query structure、derived metric declaration 等共同决定。
- `schema_version` 和 `profile` 只用于约束校验与消歧，不能覆盖 AST shape 或单独决定执行路径。
- compiler registry 不接触 raw question、routing evidence、seed、parser hints 或 prompt 裁剪信息。
- grammar fragment 表示 AST schema fragment + compiler rule set，不表示业务 handler、服务入口或语义识别入口。
- 所有 grammar fragment 共享 Draft AST -> validation -> compiled query/plan -> executor -> formatter 链路。
- 若 Validated AST shape 与 profile/schema 约束冲突，必须 validation failure 或 compiler refusal，不得按 profile 强行改写 shape。
...
generic compare 默认 8 列 display baseline。该 baseline 是 **v3.0-v3.2 only** 的旧显示口径；v3.3 explicit compare 中若用户请求收益率维度，收益率 metric 必须使用 `return_*` derived alias，并通过 derived performance phase 从 `ths_unit_nv_fund` 计算，不得使用 baseline 里的 `ths_yeild_*` 标量字段作为 strict performance 来源。
...
- v3.3 compare 的 derived performance 只允许单 period metric block；`period=all` 或 performance table 多周期行结构必须返回 `ClarificationRequired(reason=multi_period_compare_unsupported)`。
- 显式 compare 若包含收益率维度，compiler 使用 `fundcode in [...]` 拉取每个 fundcode 的 Mongo 直通字段、排名字段和完整 `ths_unit_nv_fund` 数组；derived phase 按 fundcode 计算并合并 `return_*`；formatter 沿用 compare 表格。
...
| --- | --- | --- | --- | --- |
| 近1周 | 最近一周、过去一周、一周来 | 1w | 往前 5 个交易日 | `return_1w` alias + rank fields |
| 近1月 | 最近一个月、过去一个月、这一个月 | 1m | 往前 21 个交易日 | `return_1m` alias + rank fields |
| 近3月 | 近3个月、过去三个月、最近一个季度、这三个月、前三个月 | 3m | 往前 63 个交易日 | `return_3m` alias + rank fields |
| 近6月 | 近半年、这半年、最近半年、过去六个月、半年来 | 6m | 往前 126 个交易日 | `return_6m` alias + rank fields |
| 近1年 | 近一年、最近一年、过去一年、这一年 | 1y | 往前 250 个交易日 | `return_1y` alias + rank fields |
| 近2年 | 最近两年、过去两年 | 2y | 往前 500 个交易日 | `return_2y` alias + rank fields |
| 近3年 | 最近三年、过去三年 | 3y | 往前 750 个交易日 | `return_3y` alias + rank fields |
| 近5年 | 最近五年、过去五年 | 5y | 往前 1250 个交易日 | `return_5y` alias + rank fields |
| 今年以来 | 年初以来、今年到目前、今年到现在 | ytd | T 所在年份首个交易日至 T | `return_ytd` alias + rank fields |
| 成立以来 | 成立到现在、成立至今、上市以来 | std | 首条可用净值记录至 T | `return_std` alias + rank fields |
| 各周期 / 全部周期 | 全周期、所有周期 | all | 仅 single `performance_table` 展开 10 个 `return_*` aliases；compare/list/filter/topN 必须 clarification |
| 未指定 | - | 1y | legacy display default；v3.2 strict filter/sort 不得静默采用，single performance 可采用但必须标记 default_period_used |

交易日数映射为数组位置：按 `btime` 升序排序后，最新一条为 T（索引 -1），近1周起始为索引 -6（T-5），近1月起始为索引 -22（T-21），以此类推。ytd 取 `btime` 所属年份等于 T 年份的第一条；std 取数组第一条。**计算只依赖数组位置，不依赖日期减法。**

**v3.0-v3.2**：收益率值直接查询 `ths_yeild_*` 标量字段。
**v3.3 起**：收益率由 derived phase 从 `ths_unit_nv_fund` 时间序列计算。`ths_yeild_*` 收益率标量字段不再进入 v3.3 AST select；排名字段（`ths_yeild_rank_*`）仍为数据库标量字段，正常查询。

derived return 的 Draft AST 必须显式表达 return metric + period；表达形式可以是 derived alias、metric object 或等价 AST 结构。NAV 等物理依赖由 compiler 白名单展开，并记录 provenance。`performance_rows` 只属于 derived performance 子 schema/output shape，不是 v3.3 通用顶层字段。

计算方法：

- T = `ths_unit_nv_fund` 序列按 `btime` 升序排序后的最后一条（上一个交易日），T 不是系统日期或自然日
- 对于周期 1w/1m/3m/6m/1y/2y/3y/5y：起始点为数组倒数第 N+1 条（N = 交易日数：5/21/63/126/250/500/750/1250）
- 对于 ytd：起始点为当年第一条（btime 所属年份 = T 的年份）
- 对于 std：起始点为净值序列第一条可用记录（不一定是基金成立日）；std 计算和展示起点均为该记录的 `btime`
- 收益率 = (T 的 value - 起始点的 value) / 起始点的 value × 100%
- NAV 点过滤顺序：先解析并统计非法 NAV 点到 `invalid_nav_point_count`，再从可用点序列中剔除非法点；剔除后起始点或结束点不存在则该 period 为 null。非法 NAV 点定义为：缺 `btime`、`btime` 不可解析、缺 `value`、`value` 非数值、`value <= 0`。
- 0 个有效 NAV 点、NAV 缺失或不可解析时为 `data_not_available`。NAV 至少 1 个有效点时 std 可计算；只有 1 个有效点时 std return 为 0%，`display_range` 为 `btime ~ btime`，其它固定周期为 null。
- 固定周期数据不足、ytd 无当年起点、起始值 0/null、结束值 null：对应周期 null，对应单元格显示 `暂无数据`，不得 fallback 到其它周期或旧标量字段。

单只基金 `period=all` 时，LLM Draft 的 all/全部周期语义归一展开 10 个 `return_*` aliases，并在 provenance 中标记 normalization from `period=all`。derived phase 基于同一份完整 `ths_unit_nv_fund` 序列一次性生成全部周期的收益率和时间范围。部分周期 null 不失败；对应单元格显示 `暂无数据`。

排名展示按 rank metadata：同类排名展示 `fund/origin`，ETF 排名展示 `etf/numeric`。

排序或比较时使用数字排名 `_fund`。

rank 是独立 rank grammar/field 口径：numeric rank 用于排序，origin rank 用于展示；不得和 derived return 混写。v3.2/legacy 的旧 `ths_yeild_*` return 字段可作为历史口径保留，但不得作为 v3.3 derived return strict-pass 替代。

rank metadata：

- `rank_scope: fund | etf` 和 `rank_value_kind: origin | numeric` 是 FieldSpec 固有属性。
- `rank_scope=etf` 只能搭配 `rank_value_kind=numeric`；`etf/origin` 非法。
- 同类排名展示使用 `fund/origin`，排序使用 `fund/numeric`；ETF 排名展示和排序使用 `etf/numeric`。
- `rank_audit` 必须记录 `rank_scope`、`rank_value_kind`、`period`、`rank_sort_field`、`rank_display_field`、`rank_sort_direction`。
- `rank_list` 只用于排名/名次问法。`rank_list` 不产生 `derived_audit`，但必须产生 `rank_audit`，验证 numeric rank 排序、origin/ETF rank 展示和 period/rank metadata 对齐。
- 同时包含收益率值和排名语义时走 `derived_performance_table`；收益最高、收益超过、TopN 收益必须走 `return_*` derived alias。`derived_return_list` 可携带 rank display fields。
- Registry canonical field 固定为 `ths_yeild_rank_std_etf`；`ths_yeild_std_rank_etf` 标记为 deprecated alias。若远端探针发现 canonical 不存在而 deprecated 存在，后续实现需更新 Registry 并记录 spec errata；若两者都存在但值不一致，gate blocked，不猜。
...
v3.3 derived performance strict pass 规则：

- Draft AST 必须显式声明 requested return metric + requested period；只出现 `intent=performance` 或 `profile=performance` 不足以通过。
- Validated AST 可把 derived alias 归一为 canonical derived metric，但不得新增用户未表达的 period、return metric、rank field 或 sort rule。
- compiler 可基于 Registry 白名单追加 `ths_unit_nv_fund`、rank scalar 和 display context projection，并在 provenance 中记录为 physical/compiler expansion。
- derived phase 只能计算 Draft AST 声明的 derived return，不得根据 formatter 模板补齐其它周期收益。
- 删除 Draft AST 中的 period、derived return declaration、rank field 或 `order_by` 后，validator 必须拒绝或输出 `strict_pass=false`。
- compare/list/filter/topN 遇到 `period=all` 必须 clarification，不得静默展开矩阵或退旧字段。
- formatter 不读取 `ths_unit_nv_fund`、不重算收益率。
- 删除、置空或污染 `ths_yeild_*` 收益率标量后，v3.3 performance 输出不受影响。

### v3.3 Derived Performance Universe And Audit [Hard]

derived filter/sort/topN 必须在可审计候选集合上执行。合法 bounded universe 仅包括：

- 显式 fundcode set。
- search result fundcode set，且 `search_has_more=false`。
- 非 derived Mongo where，且所有 where 字段在 Registry 标记 `bounded_universe_safe=true`。

禁止用“数据库前 N 条”临时当 universe。`return_*` filter/sort/topN 不允许 Mongo 预 limit；必须先加载完整候选集或证明 bounded universe 闭集，再执行 derived filter/sort，最后应用用户 `limit`。

category universe strict 条件：

- `bounded_universe_safe=true` 的 Mongo where 必须通过 count 查询或 cap+1 / exhausted cursor 证明 `has_more=false`。
- 若 exhausted cursor 证明无更多，可令 `candidate_count_total=loaded` 并 strict pass。
- 若 `has_more=true`，不得 strict pass，只能返回 partial result。

strict pass 规则：

- runtime output 增加 `strict_pass: true | false`。
- Strict pass 必须 `has_more=false` 且 `strict_pass=true`。
- `has_more=true` 只允许 non-strict partial result，`strict_pass=false`，必须固定提示：`基于前 {candidate_count_loaded} 条候选，可能存在更多结果`。
- single query 中 candidate counts 固定为 1。
...
```json
{
  ...
  "candidate_count_loaded": 12,
  "candidate_cap": 200,
  "has_more": false,
  "derived_value_valid_count": 10,
  "derived_value_missing_count": 2,
  "invalid_nav_count": 1,
  "invalid_nav_point_count": 4
}
```

计数规则：

- `candidate_count_total` 在 strict global query 中不得为 null；显式 fundcode set total 为 set size；search universe total 仅在 `search_has_more=false` 时为 fundcode set size。
- `derived_value_*` 按 requested metric/period 计数。
- `invalid_nav_count` 按 fund row 计数。
- `invalid_nav_point_count` 按被过滤的非法 NAV 点计数。

provenance 扩展：

- `compiler_expansions` 使用对象：`{kind, field, reason, source_profile}`。
- 必须记录 `derived_performance_inputs`、`derived_metrics`、`derived_filters`、`derived_order_by`、`derived_audit`。
- derived semantics 必须追溯到 Draft AST 的 `return_*` alias；physical inputs 只作为 compiler expansion。
...
- 在 v3.0-v3.2 `performance` profile 中，`ths_unit_nv_fund` 只作为 `display_context_for_period_range` 使用；LLM Draft 不需要选择该字段，validator/compiler 可基于 Registry 声明追加物理 projection，并在 provenance 中记录为 display context，不计为 semantic addition。v3.3 strict performance 改用 `required_physical_inputs` 声明完整 `ths_unit_nv_fund` 数组，不能把同一字段同时列入 `display_context_fields`。

#### 收益率

- `ths_yeild_1w_fund`, `ths_yeild_1m_fund`, `ths_yeild_3m_fund`, `ths_yeild_6m_fund`, `ths_yeild_1y_fund`, `ths_yeild_2y_fund`, `ths_yeild_3y_fund`, `ths_yeild_5y_fund`, `ths_yeild_ytd_fund`, `ths_yeild_std_fund`：v3.0-v3.2 legacy field catalog 中为 `selectable`, `sortable`；比较筛选能力以 profile operator gate 为准
- 新版数据字典确认以上 yield 字段为 `number` 标量。v3.0-v3.2 直接查询这些标量字段。**v3.3 strict performance 起**，收益率由 derived phase 从 `ths_unit_nv_fund` 时间序列计算，`ths_yeild_*` 收益率标量字段不再进入 v3.3 AST select/where/order_by/compare baseline（排名字段 `ths_yeild_rank_*` 除外）。收益周期字段仍必须来自 LLM AST Draft，但字段形态为 `return_*` alias object；validator 不得按 profile baseline 补语义字段。
- 数字排名字段：
...
- Registry canonical field 固定为 `ths_yeild_rank_std_etf`；数据字典同时列出的 `ths_yeild_std_rank_etf` 是 deprecated alias。若远端探针发现 canonical 不存在而 deprecated 存在，后续实现需更新 Registry 并记录 spec errata；若两者都存在但值不一致，gate blocked，不猜。
- 字符串排名字段：
...
```json
{
  ...
  "llm_draft_evidence": {
    ...
    "candidate_fields": [
      ...
      {"field": "return_ytd", "type": "derived_return", "period": "ytd", "evidence": "今年收益最高"}
    ],
    "value_candidates": [
...
- Mongo result
- `output_style`
- Registry 允许的 compiler/derived expansion metadata，例如 v3.0-v3.2 `performance_period_ranges` 或 v3.3 `performance_rows`
...
- 读取 `ths_unit_nv_fund`、计算或重算 `return_*` 收益率

### v3.3 Implementation Conformance Note [Hard]

以下实现不符合本规格：

- 把 v3.3 顶层固定为任一子 grammar，或要求所有 v3.3 AST 都包含某个子 grammar 的 required fields。
- 把 `v3.3-derived-performance` 作为 v3.3 顶层 schema，或要求所有 v3.3 AST 都包含 `performance_rows`。
- 通过 intent/profile 直接调用业务函数，而不是编译 Validated AST。
- 把 report、manager_detail、performance、composite 实现为绕过统一 AST validator/compiler 的独立 handler。
- 由 validator、compiler、executor 或 formatter 补齐 Draft AST 缺失的业务语义后仍计为 strict pass。
...
- 收益率、费率、占比保留 2 位，加 `%`
- 排名按 rank metadata 展示：同类排名展示 `fund/origin`，ETF 排名展示 `etf/numeric`
- null 或缺失字段显示 `暂无数据`
- performance 的”时间范围”和收益率值来自 compiler/derived phase 返回的结果；formatter 不得基于 `ths_unit_nv_fund` 自行计算，也不得用净值序列重算收益率。v3.3 起，derived phase 负责计算，formatter 只渲染 result row alias 字段和 `performance_rows` metadata；`performance_period_ranges` 仅为 v3.0-v3.2 legacy display metadata。
- 长文本字段使用原文截断展示，不做 LLM 摘要或改写；每个字段最多 200 字，超过时末尾加 `...（已截断）`
- 年报重仓无名称字段时只展示代码+占比
- 不做 LLM 摘要、不编造、不做投资判断

### V1 Baseline Answer Fields (v3.0-v3.2 回归基线)

v3.0-v3.2 要求保持 v1 回答 shape。Registry 确定性生成 answer_fields 时必须包含以下基线字段；可根据用户问题追加同集合 `selectable` 字段，但不得删减、替换或重排基线字段。v3.3 `performance_table` 不适用本 baseline 的收益率字段；v3.3 performance 使用 `return_*` derived alias、`performance_rows` metadata 和 rank fields。
...
| `performance` | v3.0-v3.2 only: `fundcode`(plain) + period 对应 `ths_yeild_*` 收益率字段(percent) + `_fund_origin`(plain) + `_etf`(number)；period=all 时展开所有周期 |
...
| ast_generation_mode / result bucket | 含义 | text2sql_strict_pass | question_answer_success / answer_value_success |
| --- | --- | --- |
| `deterministic_legacy` | legacy golden/debug/emergency fallback 路径 | 否；covered executable question 若出现该 mode 则 v3.2 验收失败 | 否 |
| `llm_ast_draft` | v3.2 LLM 完整 AST Draft 路径，且 validator/compiler/e2e 通过 | 是 | 是 |
| `llm_ast_draft_failed` | LLM 调用、JSON 解析、schema validation、validator 或 gate 失败 | 否，单独统计失败原因 | 否 |
| `blocked_by_verification` | 能力存在但被 gate、口径或结构验证拦截 | 否，单独统计，不能作为 question-level strict pass | 否 |
| `data_not_available` (pre-AST/pre-compiled) | gate passed 前或 compiled plan 缺失前的数据/口径不可用 | 否 | 否 |
| `data_not_available` (post-compiled/post-execution) | Draft AST、Validated AST、compiled plan provenance 完整，但当前 NAV/数组/结果为空或不可用 | 是，前提是无 semantic repair | 否 |
| `DeniedQuery` | 明确拒绝或实时/建议/大盘类边界 | 否 | 否 |
| `ClarificationRequired` | 歧义、多候选、条件不足 | 否 | 否 |
...
### `data_not_available` 分阶段统计 [Hard]

`text2sql_strict_pass` 与 `answer_value_success` 必须分开统计。

规则：

- post-compiled / post-execution `data_not_available`：若 Draft AST、Validated AST、compiled plan provenance 完整且无 semantic repair，可保留 `text2sql_strict_pass=true`，但 `answer_value_success=false`。
- partial child/data missing：只能保留成功 child 的 pass；父级 composite 不得伪装完整 answer success。
- pre-AST / pre-compiled / gate blocked / verification blocked / `blocked_by_verification`：不得计为 `text2sql_strict_pass=true`。
- `data_not_available` 不得触发 fallback 到其它 profile、handler 或 formatter-derived answer。
- question-level `data_not_available` 必须独立统计，不得混入 `llm_ast_draft_passed` 或最终 answer success。
...
#### v3.3 Release Pass Criteria [Hard]

release threshold 只能来自 verification 前已声明的 coverage matrix / release checklist / gate manifest。threshold 不得由 demo、runtime counts、单次 smoke 结果或事后统计临时定义。

规则：

- 若某 grammar fragment 没有 declared threshold，则不能声明该 fragment release pass。
- 任一 declared bucket 未独立通过，整体 release pass 为 false。
- aggregate pass rate 只能在所有 declared buckets 独立通过后作为附属展示；不得掩盖子桶失败。
- `v3_2_compatibility_baseline` pass 不得抵消 v3.3 new fragment failure。
- release report 必须至少分桶展示 `v3_2_compatibility_baseline` 与 `v3_3_new_fragments`，后者再按 declared grammar fragments 分桶。
- release pass 判定只能使用 verification 前声明的 denominator；planned blocked / future grammar application 不进入 denominator。
...
### Known Implementation Nonconformance [Audit]

本节记录当前实现相对 Hard spec 的已知偏离。它不是 waiver、backlog、compatibility mode，也不能作为 release 例外。发现 runtime 偏离 hard spec 时，必须登记，不得通过 spec hardening 合法化 runtime shortcut。

每条 nonconformance 必须包含 `category`、`affected module/file hint`、`violated hard rule`、`expected remediation class`、`release impact`、`closure condition`。

| category | affected module/file hint | violated hard rule | expected remediation class | release impact | closure condition |
| --- | --- | --- | --- | --- | --- |
| capability-level top schema_version | `etf_agent/ast_validator.py`, `etf_agent/generation_context.py`, `etf_agent/derived_performance.py`, `etf_agent/v3.py`, `tests/test_v3_3_derived_performance.py` | Runtime `ast_schema_version` 只允许协议层级值；禁止 `v3.3-derived-performance` 作为 top schema_version | Replace top schema with `v3_3_structured_query`; move capability identity to `grammar_fragment_id` / `compiler_rule_id` / metadata | Blocks v3.3 release pass | `rg "v3\\.3-derived-performance|v3_3_report_ast|v3\\.3-report-array" etf_agent tests scripts` has no top schema usage; tests assert protocol schema plus fragment metadata |
| v3.3 required fields lifted from derived fragment | `etf_agent/ast_validator.py` (`V3_3_REQUIRED_KEYS`) | 任一子 grammar required fields 不得提升为所有 v3.3 AST required fields；v3.2 base AST 是 v3.3 合法子集 | Shape-gated schema validation: require `performance_rows` only for derived performance table display shape, never for all v3.3 AST | Blocks v3.3 release pass and v3.2 compatibility baseline | v3.3 validator accepts a valid `v3_2_base_ast` subset and report/timeseries ASTs without `performance_rows`; negative tests still reject inconsistent derived display rows |
| derived performance semantics partly coupled to display/profile contract | `etf_agent/generation_context.py`, `etf_agent/ast_validator.py`, `etf_agent/derived_performance.py`, `etf_agent/formatter.py` | `performance_rows` is display contract, not query semantics source; `intent`/`profile`/`output_style` cannot infer derived metric | Require explicit derived declaration in Draft AST; validate `performance_rows` only for consistency; compile metrics only from semantic declaration | Blocks derived performance fragment pass | Removing `performance_rows` from a Draft AST with valid derived declaration does not remove compiled metrics; removing derived declaration fails validation even if `performance_rows` remains |
| v3.3 strict runtime narrowed to derived performance compiler path | `etf_agent/v3.py` (`_semantic_query_v3_3_strict`), `etf_agent/derived_performance.py` | v3.3 is unified AST grammar extension release, not a derived-performance-specific protocol or compiler | Generalize v3.3 dispatcher to shape-based compiler rules under unified AST; report/timeseries/composite fragments enter through same validator/compiler contract | Blocks overall v3.3 release pass; derived fragment may only claim fragment status after its own bucket passes | v3.3 executable path accepts declared v3.2 baseline, report, timeseries, derived, and composite buckets through protocol-level AST and reports per-bucket pass/fail |
| v3.3 tests assert noncompliant top schema | `tests/test_v3_3_derived_performance.py` | tests must not encode capability-level top schema_version or require sub grammar fields globally | Rewrite tests to assert `ast_schema_version=v3_3_structured_query` and capability metadata separately | Blocks using those tests as release evidence | Tests fail if capability-level schema is reintroduced; tests cover v3.2 base AST compatibility under v3.3 |

### Spec Consistency Audit [Test]

文档级审计必须搜索并人工判定以下关键词上下文：
...
- 不得存在 capability-level top schema_version。
- 不得存在所有 v3.3 AST 都必须携带某子 grammar required fields 的表述。
- 不得存在 profile/intent/schema_version/output shape 作为唯一 compiler selector 的表述。
- 不得存在 `performance_rows` 作为 query semantics source 的表述。
- 不得存在 v3.2 base AST 在 v3.3 下不再合法的表述。
- 若代码搜索发现当前实现违反 hard spec，必须登记到 `Known Implementation Nonconformance [Audit]`。
- 文档中出现 `v3.3-derived-performance`、`v3_3_report_ast`、`performance_rows`、`handler`、`dispatch`、`selector` 等敏感词时，只能出现在禁止、审计、历史兼容说明或明确非真源语境中。
...
- derived_performance：完整 `ths_unit_nv_fund` 数组 projection，禁止 `$slice:-1`；`return_*` alias 与 `select.type=derived_return` 一致；`answer_fields.source=derived`；`performance_rows` 与 selected derived return 一一对应且 period 一致；交易日 offset、ytd、std、`period=all` single 展开；compare/list/filter/topN 遇到 `period=all` 返回 `ClarificationRequired(reason=multi_period_compare_unsupported)`；1 个有效 NAV 点时 std=0%、其它固定周期 null；0 个有效 NAV 点或 NAV 不可解析时 `data_not_available`；非法 NAV 点计入 `invalid_nav_point_count` 后剔除；`return_*` filter/sort/topN 在 derived phase 执行且不得 Mongo 预 limit；bounded universe、candidate cap、`has_more`、`strict_pass` 和 partial result 固定提示可审计；`derived_audit` 计数粒度正确；formatter 不读取 `ths_unit_nv_fund`、不重算收益率；删除、置空或污染 `ths_yeild_*` 收益率标量后 v3.3 performance 输出不变。
- rank_list：numeric rank 排序，按 rank metadata 展示同类 `fund/origin` 与 ETF `etf/numeric`，输出 `rank_audit`，std ETF rank 使用 Registry canonical `ths_yeild_rank_std_etf`。
