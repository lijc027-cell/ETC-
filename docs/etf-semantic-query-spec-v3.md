...
Section 3 是当前阶段的唯一权威可用集；Section 4 的矩阵仅定义全集映射。任何冲突时，以 Section 3 为准。未在当前阶段白名单中的 intent 不进入 `intent_candidates`，不进入 AST。
...
本表是全版本矩阵；每阶段仅开放 Section 3 白名单内的 intent。未在当前阶段白名单中的 intent 不进入 `intent_candidates`，也不进入 AST。

| recognized_query_mode | intent (全集) | output_style | from |
| --- | --- | --- | --- |
| single | basic_info, fund_scale, tracking_index, performance, fee, manager, manager_detail, fee_and_manager, dividend, investment_profile, subscription_redemption, linked_fund | summary | tb_ths_etf_base |
| search | search | list | tb_ths_etf_base |
| filter | filter | list | tb_ths_etf_base |
| compare | compare | compare | tb_ths_etf_base |
| report | report_industry*, report_concept* | report_list | tb_ths_etf_report_quarter |
| report | report_holding*, institution_holding* | report_list | tb_ths_etf_report_year |

`*` 表示该 intent 非 v3.0 开放；开放阶段见 Section 3 的阶段白名单。
...
LLM 只能在 `intent_candidates` 和 `from_candidates` 内选择。
...
- 候选总数超过 5 个时，追加 `has_more: true` 标记；未超过时不返回此字段
...
以下 seed 是 v3 核心路由种子，配置文件只能追加同义词，不得删除、覆盖或改变其目标 intent。外部配置只允许追加 `extra_seeds`，不得覆盖 `canonical_seeds`。测试和 review 以 spec 中的 canonical seed list 为基线。
