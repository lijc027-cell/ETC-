...
v3.2 起，base 扩展字段必须通过明确的 `field_profile` 开放，而不是追加到任意 single intent：

- `basic_info` 保持 v3.0 回归基线 4 字段不变，不因 v3.2 自动扩展。
- `basic_info_extended` 负责成立日期、上市地点、业绩比较基准等单只基础扩展问法。
- `investment_profile` 负责投资目标、范围、理念、策略、风险特征。
- `subscription_redemption` 负责申赎状态。
- `linked_fund` 负责联接基金。
- `manager_detail` 负责基金经理任职起始日、任期、任职年化回报、管理规模等结构化详情。
...
| --- | --- | --- | --- |
| 基本信息 | single | `basic_info` / `basic_info_extended` / `tracking_index` / `fee` / `manager` / `fee_and_manager` | 单只 ETF 标量字段查询，仍由 AST 生成字段集合；`basic_info` 保持 v3.0 回归基线，v3.2 扩展问法走 `basic_info_extended` |
...
| v3.1 | v3.0 + search, filter, compare | report_*, basic_info_extended, manager_detail, investment_profile, subscription_redemption, linked_fund |
| v3.2 | v3.1 + basic_info_extended, manager_detail, investment_profile, subscription_redemption, linked_fund | report_* |
| v3.3 | v3.2 + report_*（仅 `verification_passed` 后开放） | 未验证通过的 report_* |
...
| `manager_detail` | same-mode | 回退到 `manager` + 明确说明 | 保留 `recognized_query_mode=single`；返回现任基金经理 + 基金管理人，并说明任期/历史业绩暂无可用结构化数据 |
...
- `where` 不由 LLM 生成；compiler 根据 `entity_hints` 注入并校验 where，LLM 仅负责可选字段与展示字段。
...
`entity_hints` 由本地确定性规则抽取，不由 LLM 直接生成。
...
- filter 中出现“收益 / 收益率 / 回报”但未指定周期时，默认映射到 `ths_yeild_1y_fund`
- performance 意图的 period 解析与 filter 的收益率默认周期一致
...
| query_mode | where 来源 | 注入规则 |
| --- | --- | --- |
| single | `entity_hints.fundcodes[0]` | `fundcode eq` |
| search | `entity_hints.search_keyword` | `__search_text__ contains` |
| filter | `entity_hints.filters` | 固定 AND 条件 |
| compare | `entity_hints.fundcodes` | `fundcode in` |
| report | `entity_hints.fundcodes` + `report_period_hint` | fundcode 条件 + report_period |

规则：

- v3.1+ 起，LLM 不生成 where。
- compiler 根据 `entity_hints` 注入 where。
- 若 LLM 输出 where，validator 必须拒绝或忽略，并以本地 `entity_hints` 为准。
...
### 6.2 Search/Filter Selection Context (v3.1+)
...
### 6.3 v3.2 Base Extension Selection Context
...
```json
{
  "field_profile": "subscription_redemption",
  "selectable_fields": [
    "fundcode",
    "ths_fund_extended_inner_short_name_fund",
    "ths_pur_and_redemp_status_fund"
  ...
  "baseline_answer_fields": [
    {"field": "fundcode", "label": "基金代码", "format": "plain"},
    {"field": "ths_fund_extended_inner_short_name_fund", "label": "基金简称", "format": "plain"},
    {"field": "ths_pur_and_redemp_status_fund", "label": "申赎状态", "format": "plain"}
  ]
}
```
...
```json
{
  ...
  "baseline_answer_fields": [
    {"field": "fundcode", "label": "基金代码", "format": "plain"},
    {"field": "ths_fund_extended_inner_short_name_fund", "label": "基金简称", "format": "plain"},
    {"field": "ths_etf_to_code_fund", "label": "联接基金", "format": "plain"}
  ]
}
```
...
- 同一 `field_profile` 内字段缺失时，formatter 逐字段显示 `暂无数据`，不得编造或用同类字段替代。
- 用户明确请求某个 v3.2 字段时，该字段必须进入 `answer_fields`；如果字段未进入当前 profile 或 gate 未通过，返回 `UnsupportedQuery` 或 profile fallback，不得只回答其他字段。
- 跨 profile 多意图不做隐式合并。例如“投资目标、申赎状态和联接基金”若未进入显式 composite，返回 `UnsupportedQuery`，不得只回答其中一项。
- `investment_profile` 不得降级为 `basic_info`。
- `subscription_redemption` 不得降级为 `basic_info` 或 `search`。
- `linked_fund` 不得降级为 `basic_info` 或 `search`。
- `manager_detail` gate 未通过时只允许 fallback 到 `manager`，且答案必须明确说明任期/历史业绩暂无可用结构化数据。
...
| 标准表达 | 变体 | period | 字段 |
| --- | --- | --- | --- |
| 近1周 | 最近一周、过去一周、一周来 | 1w | `ths_yeild_1w_fund` + 排名字段 |
| 近1月 | 最近一个月、过去一个月、这一个月 | 1m | `ths_yeild_1m_fund` + 排名字段 |
| 近3月 | 近3个月、过去三个月、最近一个季度、这三个月、前三个月 | 3m | `ths_yeild_3m_fund` + 排名字段 |
| 近6月 | 近半年、这半年、最近半年、过去六个月、半年来 | 6m | `ths_yeild_6m_fund` + 排名字段 |
| 近1年 | 近一年、最近一年、过去一年、这一年 | 1y | `ths_yeild_1y_fund` + 排名字段 |
| 近2年 | 最近两年、过去两年 | 2y | `ths_yeild_2y_fund` + 排名字段 |
| 近3年 | 最近三年、过去三年 | 3y | `ths_yeild_3y_fund` + 排名字段 |
| 近5年 | 最近五年、过去五年 | 5y | `ths_yeild_5y_fund` + 排名字段 |
| 今年以来 | 年初以来、今年到目前、今年到现在 | ytd | `ths_yeild_ytd_fund` + 排名字段 |
| 成立以来 | 成立到现在、成立至今、上市以来 | std | `ths_yeild_std_fund` + 排名字段 |
...
- 缓存按 TTL 刷新或支持手动刷新
- 缓存不可用时，不猜数据库值；前置分类保守降级为 search 或 `ClarificationRequired`
...
- `ths_fund_establishment_date_fund` 的筛选能力采用一次验证、两级开放：
  - 抽样验证通过：`selectable`、`filterable_eq`、`filterable_compare` 全部开放。
...
- `ths_fund_establishment_date_fund`：v3.2 抽样验证通过后为 `selectable`, `filterable_eq`, `filterable_compare`, `sortable`；验证未通过时仅 `selectable`
...
- orchestrator 在进入 Step 2 前，将 Step 1 结果中的 fundcode 集合写入 Step 2 子 AST 对应的 `entity_hints.fundcodes`
...
| 交易指标类 | 成交额、成交量、换手率、资金流、净现金流、融资余额、融券、融券卖出量、溢价率、折价率、委比、量比 |
| 技术分析类 | MACD、均线、RSI |
...
- “哪个费率更低 / 哪个收益更高 / 哪个规模更大”未显式多 fundcode 时归入 filter；显式多 fundcode 时归入 compare。
- “同类平均”“行业均值”“同类对比均值”这类需要 `ths_similar_fund_std_avg_yield_fund` 的问题，当前 capability matrix 未开放时返回 `UnsupportedQuery`，不得静默只回答单基金收益。
...
- 若 `order_by` 为空，第 5 列固定回退为 `ths_yeild_1y_fund`
...
| intent | baseline answer_fields |
| --- | --- |
| `basic_info_extended` | `fundcode`(plain), `ths_fund_extended_inner_short_name_fund`(plain) + 用户明确请求的 `ths_fund_establishment_date_fund`(date) / `ths_fund_listed_exchange_fund`(plain) / `ths_perf_comparative_benchmark_fund`(plain) |
| `investment_profile` | `fundcode`(plain), `ths_fund_extended_inner_short_name_fund`(plain) + 用户明确请求的投资目标/范围/理念/策略/风险特征字段(long_text) |
| `subscription_redemption` | `fundcode`(plain), `ths_fund_extended_inner_short_name_fund`(plain), `ths_pur_and_redemp_status_fund`(plain) |
| `linked_fund` | `fundcode`(plain), `ths_fund_extended_inner_short_name_fund`(plain), `ths_etf_to_code_fund`(plain) |
| `manager_detail` | gate 通过后：`fundcode`(plain), `ths_fund_extended_inner_short_name_fund`(plain), `ths_fund_manager_current_fund`(plain), `ths_fund_supervisor_fund`(plain) + 用户明确请求的任职字段；gate 未通过时 fallback 到 `manager` baseline 并说明详情不可用 |

v3.2 baseline 规则：

- 用户明确请求的字段必须进入 `answer_fields`，除非对应 gate 未通过。
- 未被用户请求的 v3.2 扩展字段不得全量追加，避免“是什么”类回答膨胀。
- `basic_info_extended` 可以一次回答成立日期、上市地点、业绩比较基准中的多个字段，但不得混入投资画像、申赎、联接基金或经理详情字段。
...
- 未验证 `ths_manager` 子字段结构前，`manager_detail` 不进入可执行 Registry；相关问法 fallback 到 `manager`，并说明任期/历史业绩暂无可用结构化数据。
- 未验证 `ths_manager.rank_num` 是否代表现任前，不允许从 `rank_num` 推断现任经理或任职顺序。
- 若验证结果表明详情字段在数组内，则 `manager_detail` 必须定义受限 `expand` 或本地结构化抽取规则；不得把数组字段当作顶层字段直接投影。
- 若验证结果表明详情字段为顶层字段，则按 `manager_detail` summary profile 展示。
- 若 gate 通过但某个详情字段无值，formatter 对该字段显示 `暂无数据`；不得用现任经理字段替代任期、历史业绩或管理规模。

## 19. 远端验证阻塞项

未验证前能力不开启：

- v3.2：成立日期字段是否稳定为 `YYYY-MM-DD`；验证通过则 `selectable + filterable_eq + filterable_compare` 全开，未通过则仅允许展示
- v3.2：`manager_detail` 依赖 `ths_manager` 子字段结构验证；未验证前不进入可执行 Registry，只允许 fallback 到 `manager`
- v3.2：`ths_manager.rank_num` 是否能代表现任顺序；未验证前不得从 rank 推断现任或任职顺序
...
| v3.2 | 至少 17 条 base 扩展 |
| v3.3 | 至少 20 条 report |
| cross-stage composite | 至少 10 条，v3.3 后统一验收 |

v3.2 黄金集至少包含：

- `basic_info_extended` 至少 3 条：成立日期、上市地点、业绩比较基准各至少 1 条。
- `investment_profile` 至少 3 条：投资目标/范围/策略/风险特征中至少覆盖 3 类。
- `manager_detail` 至少 3 条：gate 通过展示、字段缺失展示 `暂无数据`、gate 未通过 fallback 到 `manager` 各至少 1 条。
- `subscription_redemption` 至少 2 条：单只申赎状态、空值展示。
- `linked_fund` 至少 2 条：存在联接基金、无联接基金或空值展示。
- blocked / unsupported / 防静默降级至少 4 条：跨 profile 多意图、日期 filter gate 未通过、investment_profile 不降级 basic_info、linked_fund 不降级 search。
...
以下问题若超出当前 Registry 能力，不得通过静默降级回答硬过测试：

- `benchmark` 不单独开 canonical intent，默认作为 `basic_info` / base profile 的扩展字段处理；若该 profile 未放行，则返回 `UnsupportedQuery`
- `什么时候换的基金经理`、`份额最近有变化吗` 属于时间序列或历史交接分析，不在当前 v3.1 text-to-AST 承诺范围
- 同一 fundcode 下的通用 3 intent merge 不做隐式合并；需要时应走显式 composite 或返回 `UnsupportedQuery`
...
| `basic_info_extended` | 成立日期、什么时候成立、上市地点、上市交易所、在哪里上市、业绩比较基准 |
